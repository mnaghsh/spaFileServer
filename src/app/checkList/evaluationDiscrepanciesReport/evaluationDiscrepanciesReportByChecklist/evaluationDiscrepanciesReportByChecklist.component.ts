...
@Component({
  selector: 'app-evaluationDiscrepanciesReportByChecklistSp',
  templateUrl: './evaluationDiscrepanciesReportByChecklist.component.html',
  styleUrls: ['./evaluationDiscrepanciesReportByChecklist.component.css']
})
export class EvaluationDiscrepanciesReportByCheckListComponent implements OnInit {
  ...
  displayedColumns = ['number', 'NAM_CHK_HECLI', 'PreviouslyEvaluated', 'NumberOfDuties', 'NAM_PERIOD_HSRCH'];
  ...
  getAllSchedules() {
    ...
    if (this.selectedPeriod != 4) {
      ...
      this.evaluationDiscrepancies.filterEvaluationDiscrepanciesReportsByCheckList(body).subscribe(
